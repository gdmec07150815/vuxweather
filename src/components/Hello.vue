<template>
  <div>
    <x-header :right-options ="{showMore: true}" @on-click-more="showMenu = true">
      天气预报-vux版
    </x-header>
    <group>
    ...
    <group v-for="item in weather" style="align-middle">
      {{item.date}}&nbsp;{{item.fengli}}&nbsp;{{item.type}}&nbsp;{{item.low}}&nbsp;{{item.high}}
    </group>
    <x-button type = "primary" @click.native="getWeather">查询天气</x-button>
    <actionsheet:menus = "menus" v-model="showMenu" show-cancel></actionsheet>
  </div>
</template>
